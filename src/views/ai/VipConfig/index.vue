<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['ai:VipConfig:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['ai:VipConfig:export']"
          >导出</el-button
        >
      </el-col> -->
      <right-toolbar
    ...
    >
      ...
      <el-table-column label="时长(天)" align="center" prop="time" />
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listVipConfig(this.queryParams).then((response) => {
        this.VipConfigList = response.rows;
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.id != null) {
            updateVipConfig(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除vip套餐编号为"' + ids + '"的数据项？')
        .then(function () {
          return delVipConfig(ids);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      this.download(
        ...
        `VipConfig_${new Date().getTime()}.xlsx`
      );
    },
  },
};

<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['ai:SmsConfig:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['ai:SmsConfig:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="SmsConfigList" @selection-change="handleSelectionChange">
    ...
    <el-tabs v-model="tabIndex" @tab-click="handleClick">
      <el-tab-pane
        :label="item.name"
        v-for="(item, index) in SmsConfigList"
        :key="index"
        :name="String(index)"
      ></el-tab-pane>
    </el-tabs>
  ...
  methods: {
    ...
    getList() {
      ...
      listSmsConfig(this.queryParams).then((response) => {
        this.SmsConfigList = response.rows;
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateSmsConfig(this.form).then((response) => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSmsConfig(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除短信配置编号为"' + ids + '"的数据项？')
        .then(function () {
          return delSmsConfig(ids);
        })
        .then(() => {
          this.getList();
    ...
    refreshRules() {
      let row = this.SmsConfigList[Number(this.tabIndex)];
      if (row) {
